...
int OptimalPageReplacement(const vector<int> &trace,
  ...
  vector<int> calculate,diff;
  bool flag=true;
  int tcount=100;
  calculate.resize(accessTime.size(),0);
  diff.resize(accessTime.size(),0);
  for(int i=0;i<accessTime.size();i++)
  {
      int j=trace.at(accessTime.at(i));
      while(flag!=false)
      {
  ...
  {
      diff[i]=calculate.at(i)-traceIndex;
  }
  int max_value=diff.at(0),pos=0;
  for(int i=0;i<diff.size();i++)
  {
      if(diff.at(i)> max_value)
      {
          max_value=diff.at(i);
          pos=i;
      }
...
int FIFOPageReplacement(const vector<int> &trace,
  ...
  victimPageFrame= FIFO_count% frameCount;
  FIFO_count+=1;
  return victimPageFrame;  // return the page frame to replace
}
...
int LRUPageReplacement(const vector<int> &trace,
  ...
  //       number of the page to be replaced.
    ...
    int min_value=accessTime.at(0),pos=0;
  ...
  for(int i=0;i<accessTime.size();i++)
  {
      if(accessTime.at(i)< min_value)
      {
          min_value=accessTime.at(i);
          pos=i;
      }
  }
  victimPageFrame=pos;
