...
ProcessTrace::ProcessTrace(shared_ptr<mem::MMU> &memory_,
        shared_ptr<PageFrameAllocator> &allocator_,
        string file_name_, int num, int ts)
: memory(memory_), allocator(allocator_), file_name(file_name_), line_number(0), pno(num), timeslice(ts) {
...
bool ProcessTrace::ParseCommand(
    ...
    if (std::getline(trace, line)) {
        ++line_number;
        cout << std::dec << line_number << ":" << pno << ":" << line << "\n";
        ...
        cout << std::dec << line_number << ":" << pno << ":" << "TERMINATED" << "\n";
        return false;
    } else {
...
void ProcessTrace::CmdQuota(const std::string& line, const std::string& cmd, const std::vector<uint32_t>& cmdArgs) {
    max_page_size = cmdArgs.at(0);

}

void ProcessTrace::CmdAlloc(const std::string& cmd, const vector<uint32_t> &cmdArgs) {
    // Get arguments
    Addr vaddr;
    int count;
    if (cmd == "put") {
        vaddr = cmdArgs.at(0);
        count = 0x1000 / kPageSize;
    } else if (cmd == "fill") {
        vaddr = cmdArgs.at(0);
        double whateves = ((cmdArgs.at(1)*1.0) / (kPageSize * 1.0));
        count = ceil(whateves);
    }
    else if (cmd == "copy") {
        vaddr = cmdArgs.at(0);
...
void ProcessTrace::CmdPut(const string &line,
    ...
    for (int k = 0; k < num_bytes; k++) {
        try {
            ...
            memory->put_byte(addr, &buffer[k]);
            addr++;
            keep_track = k;
        } catch (PageFaultException e) {
            if (max_page_size > 0) {
                vector<uint32_t> new_cmdArgs;
                new_cmdArgs.resize(cmdArgs.size(), 0);
                new_cmdArgs.at(0) = addr;
                for (int i = 1; i < cmdArgs.size(); i++) {
                    new_cmdArgs[i] = cmdArgs[i];
                }
                CmdAlloc(cmd, new_cmdArgs);
                memory->put_byte(addr, &buffer[keep_track]);
                addr++;
                
            } else {
                cout << "ERROR: memory quota a exceeded" << endl;
...
void ProcessTrace::CmdCopy(const string &line,
    ...
    if (bytes_read != 0) {
        try {
            ...
            if (max_page_size > 0) {
                CmdAlloc(cmd, cmdArgs);
                memory->put_bytes(dst, bytes_read, buffer);
            } else {
                cout << "ERROR: memory quota exceeded"<<endl;
                delete this; // important
            }
        } catch (WritePermissionFaultException e) {
            PrintAndClearException("WritePermissionFaultException", e);
...
void ProcessTrace::CmdFill(const string &line,
    ...
    try {
        ...
        if (max_page_size > 0) {
            vector<uint32_t> new_cmdArgs;
            new_cmdArgs.resize(cmdArgs.size(), 0);
            new_cmdArgs.at(0) = addr - 1;
            uint32_t diff_fill = new_cmdArgs.at(0) - cmdArgs.at(0);
            for (int i = 1; i < cmdArgs.size(); i++) {
                if (i == 1) {
                    new_cmdArgs[i] = cmdArgs.at(i) - diff_fill;
                } else {
                    new_cmdArgs[i] = cmdArgs[i];
                }
            }
            CmdAlloc(cmd, new_cmdArgs);
            Addr new_addr = addr - 1;
            Addr diff_addr = new_cmdArgs.at(1);
            for (int i = 0; i < diff_addr; ++i) {
                memory->put_byte(new_addr++, &val);
            }
            new_cmdArgs.clear();
        } else {
            delete this; // important
...
void ProcessTrace::AllocateAndMapPage(Addr vaddr) {
    ...
    if ((l1_entry & kPTE_PresentMask) == 0) {
        ...
        allocator->Allocate(1, allocated1);
        l1_entry = allocated1[0] | kPTE_PresentMask | kPTE_WritableMask;
    ...
    allocator->Allocate(1, allocated2);
    l2_entry = allocated2[0] | kPTE_PresentMask | kPTE_WritableMask;
