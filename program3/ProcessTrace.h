...
public:
   ...
   */
  ProcessTrace(shared_ptr<mem::MMU> &memory_,
               shared_ptr<PageFrameAllocator> &allocator,
               std::string file_name_,int num,int ts);
...
private:
  ...
  // Memory contents
  std::shared_ptr<mem::MMU> &memory;
  
  // Virtual and physical mode PMCBs
  mem::PMCB vmem_pmcb;
  mem::PMCB pmem_pmcb;
  //std::vector<mem::PMCB> vmem_pmcb_array;
   ...
   */
  void CmdAlloc(const std::string &cmd,const std::vector<uint32_t> &cmdArgs);
  ...
  /**
   * 
   * @param line    -takes the string of line
   * @param cmd     -taking the operation  
   * @param cmdArgs -takes the array of strings from line
   */
